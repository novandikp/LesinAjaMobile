...
type ComponentProps = InputProps & {
  ...
  littleKeyMenuDescription: string;
  toNumber: boolean;
};
export const InputChoiceWFilter: FC<ComponentProps> = props => {
  const {
    onSelect,
    onIconPress,
    errorMessage,
    listData,
    littleKeyMenuDescription,
    keyMenuTitle,
    keyMenuDescription,
    toNumber,
  } = props;
              ...
              {searchedData.map((item, index) => {
                return (
                  <List.Item
                    ...
                    key={index}
                    title={
                      item[keyMenuTitle] + ' ' + item[littleKeyMenuDescription]
                    }
                    description={
